#!/bin/bash 
#SBATCH -o /cluster/work/vogtlab/Group/abizeul/multimodal_texture_rotations_test_firstcontent.out
#SBATCH --time=4:00:00
#SBATCH -p gpu
...
DATA_NAME="mydata_rotations_test_firstcontent"  # include mention to test/val/train, MS and C/S settings
...
python generate_clevr_dataset_latents.py --output-folder ${DIR_DATA} --n-points ${N_POINTS} --non-periodic-rotation-and-color --deterministic --all-rotations --multimodal --first_content
